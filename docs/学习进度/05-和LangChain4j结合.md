...
先看入口：[LangGraph4jDemoApplication.java](../../src/main/java/com/example/langgraph4jdemo/LangGraph4jDemoApplication.java)。

程序启动后，Spring 会自动执行 `ApplicationRunner`。这里面依次调用了：

1. 最小图
2. 条件分支
3. 循环
4. checkpoint
5. LangChain4j bridge demo

也就是说，这个项目不是靠你手动点按钮触发，而是启动后自动把每一章都跑一遍。

LangChain4j 这部分真正的入口是 `LangChainBridgeService.runDemo()`。

执行顺序是：

1. Spring 启动时，先创建 `LangChainConfig`
2. `LangChainConfig` 把 `WritingAssistant` 注册成 Bean
3. Spring 再创建 `DraftWithLangChainNode` 和 `PolishWithLangChainNode`
4. `LangChainBridgeService` 在 `@PostConstruct` 里组装图
5. `compiledGraph.stream(...)` 开始运行
6. 节点依次执行，state 不断被更新
...
- `topic / draft / finalAnswer` 是单值字段，后写覆盖前写
- `messages` 是追加型字段，每次都往列表里加
...
```java
Map.of(LangChainState.TOPIC_KEY, "LangGraph4j and LangChain4j")
```
...
1. `START` 创建初始 state
2. 进入 `draft`
3. `DraftWithLangChainNode` 读取 `topic`
4. `writingAssistant.createDraft(topic)` 生成草稿
5. 返回 `draft` 和 `messages`
6. 图把这两个字段合并进 state
7. 进入 `polish`
8. `PolishWithLangChainNode` 读取 `draft`
9. `writingAssistant.polish(draft)` 生成最终结果
10. 返回 `finalAnswer` 和 `messages`
11. 图再次合并 state
12. 到达 `END`

所以你看到的不是“一个函数一次性结束”，而是“图按节点一步一步推进”。
